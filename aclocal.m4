AC_DEFUN(AM_WITH_NETSPIRE,
[AC_ARG_WITH(netspire,
	[AC_HELP_STRING([--with-netspire=PATH],
	[set PATH to the Netspire distribution])],

	NETSPIRE_DIST=$withval)

	AC_MSG_CHECKING(for netspire-core)

	if test "z$NETSPIRE_DIST" = "z"; then
		if ! test -d "../netspire-core"; then
			AC_MSG_ERROR([Netspire distribution not found])
		else
			NETSPIRE_DIST="../netspire-core"
		fi
	else
		if ! test -d $NETSPIRE_DIST; then
			AC_MSG_ERROR([Netspire distribution not found])
		fi
	fi
	AC_MSG_RESULT(yes)
   
	AC_SUBST(NETSPIRE_DIST)
])	
